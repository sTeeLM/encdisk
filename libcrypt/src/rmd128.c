...
static INT _rmd128_compress(hash_state *md, UCHAR *buf)
#else
...
static INT rmd128_compress(hash_state *md, UCHAR *buf)
{
   ...
   burn_stack(sizeof(ULONG) * 24 + sizeof(INT));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
INT rmd128_done(hash_state * md, UCHAR *out)
{
    INT i;
    ...
    /* append the '1' bit */
    md->rmd128.buf[md->rmd128.curlen++] = (UCHAR)0x80;
    ...
    if (md->rmd128.curlen > 56) {
        while (md->rmd128.curlen < 64) {
            md->rmd128.buf[md->rmd128.curlen++] = (UCHAR)0;
        }
...
INT rmd128_test(void)
{
   ...
   for (x = 0; x < (INT)(sizeof(tests)/sizeof(tests[0])); x++) {
       rmd128_init(&md);
       rmd128_process(&md, (UCHAR *)tests[x].msg, strlen(tests[x].msg));
       rmd128_done(&md, buf);
