...
INT md2_init(hash_state *md)
{
   ...
   XZEROMEM(md->md2.X, sizeof(md->md2.X));
   XZEROMEM(md->md2.chksum, sizeof(md->md2.chksum));
   XZEROMEM(md->md2.buf, sizeof(md->md2.buf));
   md->md2.curlen = 0;
   return CRYPT_OK;
}
...
/**
   ...
   @return CRYPT_OK if successful
*/
INT md2_process(hash_state *md, const UCHAR *in, unsigned long inlen)
{
    unsigned long n;
    LTC_ARGCHK(md != NULL);
    LTC_ARGCHK(in != NULL);
...
/**
   ...
   @return CRYPT_OK if successful
*/
INT md2_done(hash_state * md, UCHAR *out)
{
    unsigned long i, k;
...
INT md2_test(void)
{
   ...
   for (i = 0; i < (INT)(sizeof(tests) / sizeof(tests[0])); i++) {
       md2_init(&md);
       md2_process(&md, (UCHAR*)tests[i].msg, (unsigned long)strlen(tests[i].msg));
       md2_done(&md, buf);
