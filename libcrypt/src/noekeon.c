...
static INT _noekeon_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
#else
...
INT noekeon_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
{
   ...
   burn_stack(sizeof(ULONG) * 5 + sizeof(INT));
   return CRYPT_OK;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static INT _noekeon_ecb_decrypt(const UCHAR *ct, UCHAR *pt, symmetric_key *skey)
#else
