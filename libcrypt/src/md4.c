...
static INT _md4_compress(hash_state *md, UCHAR *buf)
#else
static INT  md4_compress(hash_state *md, UCHAR *buf)
#endif
{
    ULONG x[16], a, b, c, d;
    INT i;
...
static INT md4_compress(hash_state *md, UCHAR *buf)
{
   ...
   burn_stack(sizeof(ULONG) * 20 + sizeof(INT));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
INT md4_done(hash_state * md, UCHAR *out)
{
    INT i;
    ...
    /* append the '1' bit */
    md->md4.buf[md->md4.curlen++] = (UCHAR)0x80;
    ...
    if (md->md4.curlen > 56) {
        while (md->md4.curlen < 64) {
            md->md4.buf[md->md4.curlen++] = (UCHAR)0;
        }
