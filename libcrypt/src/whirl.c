...
static INT _whirlpool_compress(hash_state *md, CHAR *buf)
#else
static INT whirlpool_compress(hash_state *md, CHAR *buf)
#endif
{
   ULONG64 K[2][8], T[3][8];
   INT x, y;
...
{
   LTC_ARGCHK(md != NULL);
   XZEROMEM(&md->whirlpool, sizeof(md->whirlpool));
   return CRYPT_OK;
}
...
/**
   ...
   @return CRYPT_OK if successful
*/
INT whirlpool_done(hash_state * md, UCHAR *out)
{
    INT i;
    ...
    /* append the '1' bit */
    md->whirlpool.buf[md->whirlpool.curlen++] = (CHAR)0x80;
    ...
    if (md->whirlpool.curlen > 32) {
        while (md->whirlpool.curlen < 64) {
            md->whirlpool.buf[md->whirlpool.curlen++] = (CHAR)0;
        }
   ...
   /* 512-bits, leading set of last byte */
{
  ...
  for (i = 0; i < (INT)(sizeof(tests)/sizeof(tests[0])); i++) {
      whirlpool_init(&md);
      whirlpool_process(&md, (CHAR *)tests[i].msg, tests[i].len);
