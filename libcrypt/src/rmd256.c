...
static INT _rmd256_compress(hash_state *md, UCHAR *buf)
#else
...
INT rmd256_done(hash_state * md, UCHAR *out)
{
    ...
    /* append the '1' bit */
    md->rmd256.buf[md->rmd256.curlen++] = (UCHAR)0x80;
    ...
    if (md->rmd256.curlen > 56) {
        while (md->rmd256.curlen < 64) {
            md->rmd256.buf[md->rmd256.curlen++] = (UCHAR)0;
        }
...
INT rmd256_test(void)
{
   ...
   for (x = 0; x < (INT)(sizeof(tests)/sizeof(tests[0])); x++) {
       rmd256_init(&md);
       rmd256_process(&md, (UCHAR *)tests[x].msg, strlen(tests[x].msg));
       rmd256_done(&md, buf);
