...
static INT _rmd160_compress(hash_state *md, UCHAR *buf)
#else
static INT  rmd160_compress(hash_state *md, UCHAR *buf)
#endif
{
   ULONG aa,bb,cc,dd,ee,aaa,bbb,ccc,ddd,eee,X[16];
   INT i;
...
static INT rmd160_compress(hash_state *md, UCHAR *buf)
{
   ...
   burn_stack(sizeof(ULONG) * 26 + sizeof(INT));
   return err;
}
#endif
...
INT rmd160_done(hash_state * md, UCHAR *out)
{
    ...
    /* append the '1' bit */
    md->rmd160.buf[md->rmd160.curlen++] = (UCHAR)0x80;
    ...
    if (md->rmd160.curlen > 56) {
        while (md->rmd160.curlen < 64) {
            md->rmd160.buf[md->rmd160.curlen++] = (UCHAR)0;
        }
...
INT rmd160_test(void)
{
   ...
   for (x = 0; x < (INT)(sizeof(tests)/sizeof(tests[0])); x++) {
       rmd160_init(&md);
       rmd160_process(&md, (UCHAR *)tests[x].msg, strlen(tests[x].msg));
       rmd160_done(&md, buf);
