...
INT ctr_encrypt(const UCHAR *pt, UCHAR *ct, ULONG len, symmetric_CTR *ctr)
{
   ...
   if ((ctr->padlen == ctr->blocklen) && cipher_descriptor[ctr->cipher].accel_ctr_encrypt != NULL && (len >= (ULONG)ctr->blocklen)) {
      ...
      if (ctr->padlen == ctr->blocklen) {
         ...
         if (ctr->mode == CTR_COUNTER_LITTLE_ENDIAN) {
            ...
            for (x = 0; x < ctr->ctrlen; x++) {
               ctr->ctr[x] = (ctr->ctr[x] + (UCHAR)1) & (UCHAR)255;
               if (ctr->ctr[x] != (UCHAR)0) {
                  break;
               }
            }
         } else {
...
#ifdef LTC_FAST
      if (ctr->padlen == 0 && len >= (ULONG)ctr->blocklen) {
         for (x = 0; x < ctr->blocklen; x += sizeof(LTC_FAST_TYPE)) {
            *((LTC_FAST_TYPE*)((UCHAR *)ct + x)) = *((LTC_FAST_TYPE*)((UCHAR *)pt + x)) ^
                                                           *((LTC_FAST_TYPE*)((UCHAR *)ctr->pad + x));
         }
       pt         += ctr->blocklen;
       ct         += ctr->blocklen;
       len        -= ctr->blocklen;
