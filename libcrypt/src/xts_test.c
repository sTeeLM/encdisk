...
/* #21, 32 byte key, 31 byte PTX */
{
   ...
   for (i = 0; i < (INT)(sizeof(tests)/sizeof(tests[0])); i++) {
       ...
       err = xts_encrypt(tests[i].PTX, tests[i].PTLEN, MYOUT, T, &xts);
       if (err != CRYPT_OK) {
          xts_done(&xts);
          return err;
       }

       if (XMEMCMP(MYOUT, tests[i].CTX, tests[i].PTLEN)) {
          xts_done(&xts);
          return CRYPT_FAIL_TESTVECTOR;
       }

       err = xts_decrypt(tests[i].CTX, tests[i].PTLEN, MYOUT, T, &xts);
       if (err != CRYPT_OK) {
          xts_done(&xts);
          return err;
       }

       if (XMEMCMP(MYOUT, tests[i].PTX, tests[i].PTLEN)) {
          xts_done(&xts);
          return CRYPT_FAIL_TESTVECTOR;
       }
       xts_done(&xts);
