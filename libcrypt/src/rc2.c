...
INT rc2_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
{
   ...
   for (i = 0; i < keylen; i++) {
    ...
    for (i = 0; i < 64; i++) {
        xkey[i] =  (ULONG)tmp[2*i] + ((ULONG)tmp[2*i+1] << 8);
    }        

#ifdef LTC_CLEAN_STACK
    XZEROMEM(tmp, sizeof(tmp));
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static INT _rc2_ecb_encrypt( const UCHAR *pt,
                            UCHAR *ct,
                            symmetric_key *skey)
#else
INT rc2_ecb_encrypt( const UCHAR *pt,
                            UCHAR *ct,
                            symmetric_key *skey)
#endif
{
    ULONG *xkey;
    ULONG x76, x54, x32, x10, i;
    ...
    x76 = ((ULONG)pt[7] << 8) + (ULONG)pt[6];
    x54 = ((ULONG)pt[5] << 8) + (ULONG)pt[4];
    x32 = ((ULONG)pt[3] << 8) + (ULONG)pt[2];
    x10 = ((ULONG)pt[1] << 8) + (ULONG)pt[0];
                            ...
                            symmetric_key *skey)
{
    ...
    burn_stack(sizeof(ULONG *) + sizeof(ULONG) * 5);
    return err;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static INT _rc2_ecb_decrypt( const UCHAR *ct,
                            UCHAR *pt,
                            symmetric_key *skey)
#else
INT rc2_ecb_decrypt( const UCHAR *ct,
                            UCHAR *pt,
                            symmetric_key *skey)
#endif
{
    ULONG x76, x54, x32, x10;
    ULONG *xkey;
    INT i;
    ...
    x76 = ((ULONG)ct[7] << 8) + (ULONG)ct[6];
    x54 = ((ULONG)ct[5] << 8) + (ULONG)ct[4];
    x32 = ((ULONG)ct[3] << 8) + (ULONG)ct[2];
    x10 = ((ULONG)ct[1] << 8) + (ULONG)ct[0];
                            ...
                            symmetric_key *skey)
{
    ...
    burn_stack(sizeof(ULONG *) + sizeof(ULONG) * 4 + sizeof(INT));
    return err;
}
#endif
