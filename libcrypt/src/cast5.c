...
#ifdef _MSC_VER
   #define GB(x, i) ((UCHAR)((x[(15-i)>>2])>>(unsigned)(8*((15-i)&3))))
#else   
...
static INT _cast5_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
#else
...
INT cast5_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
{
   ...
   burn_stack(sizeof(ULONG)*8 + 16 + sizeof(INT)*2);
   return z;
}
#endif
...
INLINE static ULONG FI(ULONG R, ULONG Km, ULONG Kr)
{
   ULONG I;
   I = (Km + R);
   I = ROL(I, Kr);
...
INLINE static ULONG FII(ULONG R, ULONG Km, ULONG Kr)
{
   ULONG I;
   I = (Km ^ R);
   I = ROL(I, Kr);
...
INLINE static ULONG FIII(ULONG R, ULONG Km, ULONG Kr)
{
   ULONG I;
   I = (Km - R);
   I = ROL(I, Kr);
...
*/
#ifdef LTC_CLEAN_STACK
static INT _cast5_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
#else
...
*/
#ifdef LTC_CLEAN_STACK
static INT _cast5_ecb_decrypt(const UCHAR *ct, UCHAR *pt, symmetric_key *skey)
#else
