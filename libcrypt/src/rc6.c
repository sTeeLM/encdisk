...
static INT _rc6_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
#else
INT rc6_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
#endif
{
    ULONG L[64], S[50], A, B, i, j, v, s, l;
...
*/
#ifdef LTC_CLEAN_STACK
static INT _rc6_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
#else
...
INT rc6_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
{
   ...
   burn_stack(sizeof(ULONG) * 6 + sizeof(INT));
   return err;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static INT _rc6_ecb_decrypt(const UCHAR *ct, UCHAR *pt, symmetric_key *skey)
#else
