...
static ULONG FI( ULONG in, ULONG subkey )
{
   ULONG nine, seven;
   static const ULONG S7[128] = {
  ...
  nine  = (ULONG)(in>>7)&0x1FF;
  seven = (ULONG)(in&0x7F);

  /* Now run the various operations */
  nine   = (ULONG)(S9[nine] ^ seven);
  seven  = (ULONG)(S7[seven] ^ (nine & 0x7F));
  seven ^= (subkey>>9);
  nine  ^= (subkey&0x1FF);
  nine   = (ULONG)(S9[nine] ^ seven);
  seven  = (ULONG)(S7[seven] ^ (nine & 0x7F));
  return (ULONG)(seven<<9) + nine;
}

static ULONG FO( ULONG in, INT round_no, symmetric_key *key)
{
   ULONG left, right;

  /* Split the input into two 16-bit words */
  left = (ULONG)(in>>16);
  right = (ULONG) in&0xFFFF;
  ...
  return (((ULONG)right)<<16)+left;
}

static ULONG FL( ULONG in, INT round_no, symmetric_key *key )
{
    ULONG l, r, a, b;
    ...
    a = (ULONG) (l & key->kasumi.KLi1[round_no]);
    r ^= ROL16(a,1);
    b = (ULONG)(r | key->kasumi.KLi2[round_no]);
    l ^= ROL16(b,1);
...
{
    static const ULONG C[8] = { 0x0123,0x4567,0x89AB,0xCDEF, 0xFEDC,0xBA98,0x7654,0x3210 };
    ULONG ukey[8], Kprime[8];
    INT n;
    ...
    for (n = 0; n < 8; n++ ) {
        ukey[n] = (((ULONG)key[2*n]) << 8) | key[2*n+1];
    }
