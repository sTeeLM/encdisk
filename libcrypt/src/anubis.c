...
static INT _anubis_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
#else
...
INT  anubis_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
{
  ...
  burn_stack(sizeof(INT) * 5 + sizeof(ULONG) * (MAX_N + MAX_N + 5));
  return err;
}
#endif
  

static void anubis_crypt(const UCHAR *plaintext, UCHAR *ciphertext,
                         ULONG roundKey[18 + 1][4], INT R) {
   INT i, pos, r;
   ULONG state[4];
   ULONG inter[4];
    ...
    */
    for (i = 0, pos = 0; i < 4; i++, pos += 4) {
        ULONG w = inter[i];
        ciphertext[pos    ] = (UCHAR)(w >> 24);
        ciphertext[pos + 1] = (UCHAR)(w >> 16);
        ciphertext[pos + 2] = (UCHAR)(w >>  8);
        ciphertext[pos + 3] = (UCHAR)(w      );
    }
}
