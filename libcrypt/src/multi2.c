...
static void pi1(ULONG *p)
{
   p[1] ^= p[0];
}

static void pi2(ULONG *p, ULONG *k)
{
   ULONG t;
...
static void pi3(ULONG *p, ULONG *k)
{
   ULONG t;
   t = p[0] + k[1];
...
static void pi4(ULONG *p, ULONG *k)
{
   ULONG t;
...
static void setup(ULONG *dk, ULONG *k, ULONG *uk)
{
   INT n, t;
   ULONG p[2];
...
static void encrypt(ULONG *p, INT N, ULONG *uk)
{
   INT n, t;
   for (t = n = 0; ; ) {
      pi1(p); if (++n == N) break;       
...
static void decrypt(ULONG *p, INT N, ULONG *uk)
{
   INT n, t;
   for (t = 4*((N&1)^1), n = N; ;  ) {
      switch (n >= 4 ? 4 : 0) {
