...
#define sbox(i, x) ((ULONG)SBOX[i][(x)&255])

#else

/* The Q-box tables */
static const UCHAR qbox[2][4][16] = {
{
...
static ULONG _sbox(INT i, ULONG x)
#else
static ULONG sbox(INT i, ULONG x)
#endif
{
   UCHAR a0,b0,a1,b1,a2,b2,a3,b3,a4,b4,y;
...
/* computes ab mod p */
static ULONG gf_mult(ULONG a, ULONG b, ULONG p)
{
   ULONG result, B[2], P[2];
...
#ifndef LTC_TWOFISH_TABLES
static ULONG mds_column_mult(UCHAR in, INT col)
{
   ULONG x01, x5B, xEF;
...
static void mds_mult(const UCHAR *in, UCHAR *out)
{
  INT x;
  ULONG tmp;
  for (tmp = x = 0; x < 4; x++) {
...
static void rs_mult(const UCHAR *in, UCHAR *out)
{
   ULONG tmp;
...
/* computes h(x) */
static void h_func(const UCHAR *in, UCHAR *out, UCHAR *M, INT k, INT offset)
{
  INT x;
  UCHAR y[4];
  for (x = 0; x < 4; x++) {
  ...
  switch (k) {
     case 4:
            y[0] = (UCHAR)(sbox(1, (ULONG)y[0]) ^ M[4 * (6 + offset) + 0]);
            y[1] = (UCHAR)(sbox(0, (ULONG)y[1]) ^ M[4 * (6 + offset) + 1]);
            y[2] = (UCHAR)(sbox(0, (ULONG)y[2]) ^ M[4 * (6 + offset) + 2]);
            y[3] = (UCHAR)(sbox(1, (ULONG)y[3]) ^ M[4 * (6 + offset) + 3]);
     case 3:
            y[0] = (UCHAR)(sbox(1, (ULONG)y[0]) ^ M[4 * (4 + offset) + 0]);
            y[1] = (UCHAR)(sbox(1, (ULONG)y[1]) ^ M[4 * (4 + offset) + 1]);
            y[2] = (UCHAR)(sbox(0, (ULONG)y[2]) ^ M[4 * (4 + offset) + 2]);
            y[3] = (UCHAR)(sbox(0, (ULONG)y[3]) ^ M[4 * (4 + offset) + 3]);
     case 2:
            y[0] = (UCHAR)(sbox(1, sbox(0, sbox(0, (ULONG)y[0]) ^ M[4 * (2 + offset) + 0]) ^ M[4 * (0 + offset) + 0]));
            y[1] = (UCHAR)(sbox(0, sbox(0, sbox(1, (ULONG)y[1]) ^ M[4 * (2 + offset) + 1]) ^ M[4 * (0 + offset) + 1]));
            y[2] = (UCHAR)(sbox(1, sbox(1, sbox(0, (ULONG)y[2]) ^ M[4 * (2 + offset) + 2]) ^ M[4 * (0 + offset) + 2]));
            y[3] = (UCHAR)(sbox(0, sbox(1, sbox(1, (ULONG)y[3]) ^ M[4 * (2 + offset) + 3]) ^ M[4 * (0 + offset) + 3]));
  }
  mds_mult(y, out);
}
...
#ifdef LTC_CLEAN_STACK
static ULONG _g_func(ULONG x, symmetric_key *key)
#else
static ULONG g_func(ULONG x, symmetric_key *key)
#endif
{
   UCHAR g, i, y, z;
   ULONG res;
...
static ULONG g_func(ULONG x, symmetric_key *key)
{
    ULONG y;
    y = _g_func(x, key);
    burn_stack(sizeof(UCHAR) * 4 + sizeof(ULONG));
    return y;
}
#endif /* LTC_CLEAN_STACK */
 ...
 */
#ifdef LTC_CLEAN_STACK
static INT _twofish_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
#else
INT twofish_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
#endif
{
#ifndef LTC_TWOFISH_SMALL
   UCHAR S[4*4], tmpx0, tmpx1;
#endif
   INT k, x, y;
   UCHAR tmp[4], tmp2[4], M[8*4];
   ULONG A, B;
   ...
   if (k == 2) {
        for (x = 0; x < 256; x++) {
           tmpx0 = (UCHAR)sbox(0, x);
           tmpx1 = (UCHAR)sbox(1, x);
...
INT twofish_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
{
   ...
   burn_stack(sizeof(INT) * 7 + sizeof(UCHAR) * 56 + sizeof(ULONG) * 2);
   return x;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static INT _twofish_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
#else
INT twofish_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
#endif
{
    ULONG a,b,c,d,ta,tb,tc,td,t1,t2, *k;
    INT r;
...
INT twofish_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
{
   ...
   burn_stack(sizeof(ULONG) * 10 + sizeof(INT));
   return err;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static INT _twofish_ecb_decrypt(const UCHAR *ct, UCHAR *pt, symmetric_key *skey)
#else
