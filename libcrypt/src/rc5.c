...
static INT _rc5_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
#else
INT rc5_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
#endif
{
    ULONG L[64], *S, A, B, i, j, v, s, t, l;
    ...
    if ((keylen & 3) != 0) { 
       A <<= (ULONG)((8 * (4 - (keylen&3)))); 
       L[j++] = BSWAP(A);
    }

    /* setup the S array */
    t = (ULONG)(2 * (num_rounds + 1));
...
INT rc5_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
{
   ...
   burn_stack(sizeof(ULONG) * 122 + sizeof(INT));
   return x;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static INT _rc5_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
#else
...
INT rc5_ecb_encrypt(const UCHAR *pt, UCHAR *ct, symmetric_key *skey)
{
   ...
   burn_stack(sizeof(ULONG) * 2 + sizeof(INT));
   return err;
}
#endif
...
*/
#ifdef LTC_CLEAN_STACK
static INT _rc5_ecb_decrypt(const UCHAR *ct, UCHAR *pt, symmetric_key *skey)
#else
