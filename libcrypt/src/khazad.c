...
INT khazad_setup(const UCHAR *key, INT keylen, INT num_rounds, symmetric_key *skey)
{
   ...
   S = T7;
    ...
    /*
    * map UCHAR array cipher key to initial key state (mu):
    */
   K2 =
      ((ULONG64)key[ 0] << 56) ^
      ((ULONG64)key[ 1] << 48) ^
...
static void khazad_crypt(const UCHAR *plaintext, UCHAR *ciphertext,
    ...
    */
   ciphertext[0] = (UCHAR)(state >> 56);
   ciphertext[1] = (UCHAR)(state >> 48);
   ciphertext[2] = (UCHAR)(state >> 40);
   ciphertext[3] = (UCHAR)(state >> 32);
   ciphertext[4] = (UCHAR)(state >> 24);
   ciphertext[5] = (UCHAR)(state >> 16);
   ciphertext[6] = (UCHAR)(state >>  8);
   ciphertext[7] = (UCHAR)(state      );
}
