...
static INT _rmd320_compress(hash_state *md, UCHAR *buf)
#else
static INT  rmd320_compress(hash_state *md, UCHAR *buf)
#endif
{
   ULONG aa,bb,cc,dd,ee,aaa,bbb,ccc,ddd,eee,tmp,X[16];
   INT i;
...
static INT rmd320_compress(hash_state *md, UCHAR *buf)
{
   ...
   burn_stack(sizeof(ULONG) * 27 + sizeof(INT));
   return err;
}
#endif
...
/**
   ...
   @return CRYPT_OK if successful
*/
INT rmd320_done(hash_state * md, UCHAR *out)
{
    INT i;
    ...
    /* append the '1' bit */
    md->rmd320.buf[md->rmd320.curlen++] = (UCHAR)0x80;
    ...
    if (md->rmd320.curlen > 56) {
        while (md->rmd320.curlen < 64) {
            md->rmd320.buf[md->rmd320.curlen++] = (UCHAR)0;
        }
...
INT rmd320_test(void)
{
   ...
   for (x = 0; x < (INT)(sizeof(tests)/sizeof(tests[0])); x++) {
       rmd320_init(&md);
       rmd320_process(&md, (UCHAR *)tests[x].msg, strlen(tests[x].msg));
       rmd320_done(&md, buf);
