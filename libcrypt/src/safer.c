...
/******************* Macros ***************************************************/
#define ROL8(x, n)   ((UCHAR)((ULONG)(x) << (n)\
                                     |(ULONG)((x) & 0xFF) >> (8 - (n))))
...
/******************* Types ****************************************************/
extern const UCHAR safer_ebox[], safer_lbox[];

#ifdef LTC_CLEAN_STACK
static void _Safer_Expand_Userkey(const UCHAR *userkey_1,
                                 const UCHAR *userkey_2,
                                 ULONG nof_rounds,
                                 INT strengthened,
                                 safer_key_t key)
#else
static void Safer_Expand_Userkey(const UCHAR *userkey_1,
                                 const UCHAR *userkey_2,
                                 ULONG nof_rounds,
                                 INT strengthened,
                                 safer_key_t key)
#endif
{   ULONG i, j, k;
    UCHAR ka[LTC_SAFER_BLOCK_LEN + 1];
    UCHAR kb[LTC_SAFER_BLOCK_LEN + 1];

    if (LTC_SAFER_MAX_NOF_ROUNDS < nof_rounds)
        nof_rounds = LTC_SAFER_MAX_NOF_ROUNDS;
    *key++ = (UCHAR)nof_rounds;
    ka[LTC_SAFER_BLOCK_LEN] = (UCHAR)0;
    kb[LTC_SAFER_BLOCK_LEN] = (UCHAR)0;
    k = 0;
                                 ...
                                 safer_key_t key)
{
   ...
   burn_stack(sizeof(UCHAR) * (2 * (LTC_SAFER_BLOCK_LEN + 1)) + sizeof(ULONG)*2);
}
#endif

INT safer_k64_setup(const UCHAR *key, INT keylen, INT numrounds, symmetric_key *skey)
{
   LTC_ARGCHK(key != NULL);
   ...
   Safer_Expand_Userkey(key, key, (ULONG)(numrounds != 0 ?numrounds:LTC_SAFER_K64_DEFAULT_NOF_ROUNDS), 0, skey->safer.key);
   return CRYPT_OK;
}
   
INT safer_sk64_setup(const UCHAR *key, INT keylen, INT numrounds, symmetric_key *skey)
{
   LTC_ARGCHK(key != NULL);
   LTC_ARGCHK(skey != NULL);
   ...
   Safer_Expand_Userkey(key, key, (ULONG)(numrounds != 0 ?numrounds:LTC_SAFER_SK64_DEFAULT_NOF_ROUNDS), 1, skey->safer.key);
   return CRYPT_OK;
}

INT safer_k128_setup(const UCHAR *key, INT keylen, INT numrounds, symmetric_key *skey)
{
   LTC_ARGCHK(key != NULL);
   LTC_ARGCHK(skey != NULL);
   ...
   Safer_Expand_Userkey(key, key+8, (ULONG)(numrounds != 0 ?numrounds:LTC_SAFER_K128_DEFAULT_NOF_ROUNDS), 0, skey->safer.key);
   return CRYPT_OK;
}

INT safer_sk128_setup(const UCHAR *key, INT keylen, INT numrounds, symmetric_key *skey)
{
   LTC_ARGCHK(key != NULL);
   LTC_ARGCHK(skey != NULL);
   ...
   Safer_Expand_Userkey(key, key+8, (ULONG)(numrounds != 0?numrounds:LTC_SAFER_SK128_DEFAULT_NOF_ROUNDS), 1, skey->safer.key);
   return CRYPT_OK;
}

#ifdef LTC_CLEAN_STACK
static INT _safer_ecb_encrypt(const UCHAR *block_in,
                             UCHAR *block_out,
                             symmetric_key *skey)
#else
INT safer_ecb_encrypt(const UCHAR *block_in,
                             UCHAR *block_out,
                             symmetric_key *skey)
#endif
                             ...
                             symmetric_key *skey)
{
    ...
    burn_stack(sizeof(UCHAR) * 9 + sizeof(ULONG) + sizeof(UCHAR *));
    return err;
}
#endif

#ifdef LTC_CLEAN_STACK
static INT _safer_ecb_decrypt(const UCHAR *block_in,
                             UCHAR *block_out,
                             symmetric_key *skey)
#else
INT safer_ecb_decrypt(const UCHAR *block_in,
                             UCHAR *block_out,
                             symmetric_key *skey)
#endif
{   UCHAR a, b, c, d, e, f, g, h, t;
