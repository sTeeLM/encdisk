...
FlushEncCache(
    HANDLE ImageFile, 
    PCRYPT_CONTEXT Context, 
    PENC_CACHE_BUFFER Cache)
{
    IO_STATUS_BLOCK IoStatus;
    LARGE_INTEGER  ByteOffset;

    KdPrint(("EncDisk: FlushBuffer request %d %d %I64u\n", Cache->valid, Cache->dirty, Cache->index));
    ...
    if(Cache->valid && Cache->dirty) 
    {
        ...
        /* encrypt Cache */
        if(CryptEncryptCluster(Context, Cache->plain, Cache->cipher, Cache->index) != CRYPT_OK)
        {
            return STATUS_INTERNAL_ERROR;
        }

        /* raw write disk */
        ByteOffset.QuadPart = Cache->index * CRYPT_CLUSTER_SIZE;
        ZwWriteFile(
            ImageFile,
            NULL,
            NULL,
    ...
    )
{
    ...
    if(Cache->valid && ClusterIndex == Cache->index) 
    {
        KdPrint(("EncDisk: EncryptWriteCluster hit cache\n"));
        RtlCopyMemory(&Cache->plain[Offset], Buffer, Length);
        Cache->dirty = TRUE;
    }
    else /* !Cache->valid || ClusterIndex != Cache->index */
    {
        KdPrint(("EncDisk: EncryptWriteCluster miss cache request %I64u, we have %d %d %I64u\n", 
            ClusterIndex, Cache->valid, Cache->dirty, Cache->index));
        /* we need load cache , flush old cache first */
        if(ClusterIndex != Cache->index) 
        {
            IoStatus.Status = FlushEncCache(ImageFile, Context, Cache);
            if(!NT_SUCCESS(IoStatus.Status)) 
            {
                return IoStatus.Status;
            }
        ...
        if(Length != CRYPT_CLUSTER_SIZE) 
        {
            ...
            /* decrypt it! */
            if(CryptDecryptCluster(Context, Cache->cipher, Cache->plain, ClusterIndex) != CRYPT_OK) 
            {
    ...
    )
{
    ...
    if(Cache->valid && ClusterIndex == Cache->index) 
    {
        ...
        RtlCopyMemory(Buffer, &Cache->plain[Offset], Length);
    }
    else /* !Cache->valid || ClusterIndex != Cache->index */
    {
...
{
    PUCHAR P = Buffer;
    ULONGLONG  ClusterIndex = (ByteOffset->QuadPart / CRYPT_CLUSTER_SIZE);
    ULONG Begin = (ULONG)(ByteOffset->QuadPart - (ClusterIndex * CRYPT_CLUSTER_SIZE));
    ULONG ToRead;
    ...
    while(Length > 0) 
    {
        ...
        Status->Status = DecryptReadCluster(ImageFile, Context, Cache, P, ClusterIndex, Begin, ToRead);
    ...
    PLARGE_INTEGER ByteOffset)
{
    ...
    while(Length > 0) 
    {
        ...
        Status->Status = EncryptWriteCluster(ImageFile, Context, Cache, P, ClusterIndex, Begin, ToWrite);
        if(!NT_SUCCESS(Status->Status)) 
        {
                 ...
                 )
{
    ...
    {
    if(pLUExt->Encrypt) {
        FlushEncCache(pLUExt->ImageFile, &pLUExt->EncContext, 
            &pLUExt->EncCache);
        CryptCleanupContext(&pLUExt->EncContext);
        RtlZeroMemory(&pLUExt->EncCache, sizeof(pLUExt->EncCache));
    }
		 ...
		 IN PETHREAD ClientThread)
{
  ...
  KdPrint
    ...
    // initilize enc context
    if(IMSCSI_ENCRYPT(CreateData->Flags)) {
        LUExtension->Encrypt = TRUE;
        RtlCopyMemory(&LUExtension->EncContext.key, &CreateData->EncKey, sizeof(LUExtension->EncContext.key));
        RtlZeroMemory(&LUExtension->EncCache, sizeof(LUExtension->EncCache));

        if(CryptRestoreContext(&LUExtension->EncContext) != CRYPT_OK) {
          KdPrint(("PhDskMnt: Restore CryptContext failed.\n "));
